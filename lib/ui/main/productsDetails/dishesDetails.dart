...
import 'package:shopping/model/server/reviews.dart';
import 'package:shopping/ui/main/home/home.dart';
import 'package:shopping/ui/main/mainscreen.dart';
import 'package:shopping/ui/main/productsDetails/humorist.dart';
import 'package:shopping/ui/main/productsDetails/pagin1.dart';
import 'package:shopping/ui/main/productsDetails/rProducts.dart';
import 'package:shopping/ui/main/productsDetails/restaurantWidgets.dart';
import 'package:shopping/ui/main/productsDetails/variants.dart';
...
import 'package:shopping/widget/rewiewsBar.dart';
...
class _DishesDetailsScreenState extends State<DishesDetailsScreen>
  ...
  _extrasClick(String id, bool value) {
    for (var item in _this.extras) if (item.id == id) item.select = value;
  ...
  _tapAddToCart() {
    ...
    if (!account.isAuth()) return route.push(context, "/login");
    if (basket.restaurantCheck(_this.restaurant)) {
      if (basket.dishInBasket(_this.id))
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content:
                Text(strings.get(131)))); // "This food already added to cart",
      else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(strings.get(130)))); // "This food was added to cart",
        basket.add(_this);
        _redraw();
        account.redraw();
      }
  ...
  _callbackDone() {
    ...
    reviewsFoodAdd(account.token, _this.id.toString(), _stars.toString(),
        editControllerReview.text, _success, _error);
  }
        ...
        body: Directionality(
          ...
          child: Stack(children: [
            NestedScrollView(
                ...
                body: Stack(
                  children: <Widget>[
                    ...
                    skinProductDetailsAddToCartButtons(
                        windowWidth,
                        basket.makePriceString(basket.getItemPrice(_this)),
                        _tapAddToCart,
                        _onPress),
                    IEasyDialog2(
            ...
            skinHeaderBackButtonWithBasket(
                context, mainScreenState.onBack, Colors.white),
          ]),
        ));
  }
  ...
  _children() {
    ...
    var _dPrice = "";
    var _price = "";
    if (_this.price != null) _price = basket.makePriceString(_this.price);
    ...
    if (_this.variants != null)
      for (var item in _this.variants) if (item.select) _addedName = item.name;
    ...
    if (_this.desc.isNotEmpty) {
      list.add(Container(
        margin: EdgeInsets.only(left: 10, right: 10),
        child: Text(_this.desc.replaceAll(RegExp(r'<[^>]*>|&[^;]+;'), '\n'), style: theme.text14),
        // dish description
      ));
      list.add(SizedBox(
        height: 20,
      ));
      removeAllHtmlTags(_this.desc).replaceAll(RegExp(r'<[^>]*>|&[^;]+;'), '\n');
    }
  ...
  _imageBuild() {
    ...
    listContain.add(Hero(
        tag: idHeroes,
        child: cacheImageWidgetContain(
            "$serverImages${_this.image}", theme.colorPrimary)));
    list.add(Hero(
        tag: idHeroes,
    ...
    for (var item in _this.imagesFiles) {
      listContain.add(
          cacheImageWidgetContain("$serverImages$item", theme.colorPrimary));
      ...
      children: [
        InkWell(
            onTap: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => ImagesBigScreen(list: listContain),
                ),
              );
            },
            child: TabBarView(controller: _tabController, children: list)),
        if (_this.discountprice != 0)
          Container(
            margin: EdgeInsets.only(
                left: 10,
            ...
            child: Humorist(
                child: saleSticker(
                    windowWidth * 0.5,
                    _this.discount,
                    basket.makePriceString(_this.discountprice),
                    basket.makePriceString(_this.price))),
          ),
        if (account.isAuth())
          (Container(
            alignment: Alignment.topRight,
            margin: EdgeInsets.only(
                top: 50 + MediaQuery.of(context).padding.top, right: 10),
            child: Stack(
              children: <Widget>[
                Container(
                    width: 30,
                    ...
                    alignment: Alignment.center,
                    child: Icon(
                      (account.getFavoritesState(_this.id))
                          ? Icons.favorite
        ...
        if (_this.imagesFiles.length != 0)
          Container(
            ...
            child: pagination1(
                _this.imagesFiles.length + 1, _tabIndex, theme.colorPrimary),
  ...
  _nutrition(List<Widget> list) {
    if (_this.nutritions == null || _this.nutritions.isEmpty) return;
    list.add(Container(
      margin: EdgeInsets.only(left: 10, right: 10),
      child: ListWithIcon(
          imageAsset: "assets/brands.png",
          text: strings.get(80), // Nutrition
          imageColor: theme.colorDefaultText),
    ));
    ...
    for (var data in _this.nutritions) {
      items.add(
        _nutritionItem(data.name, data.desc),
      );
      index++;
      if (index == 4) {
        index = 4;
    ...
    return Container(
        ...
        child: Column(
          children: [
            ...
            // "You can add to cart, only dishes from single restaurant. Do you want to reset cart? And add new dishes.",
            SizedBox(
              height: 50,
            ),
            Container(
